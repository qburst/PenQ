...
function importAttackFromXMLString(fileContents, container) {
    ...
    if(dom.documentElement.nodeName == "parsererror"){
        var errorText = dom.documentElement.textContent;
        errorText = errorText.replace(/Location:[ A-Za-z./:]*\n/,"");
        //the Location information is useless to the user as it refers to a
        //chrome url.
        errorText = errorText.replace(/-+\^/, "");
        //remove that --^ arrow
        alert("No attacks added.\n"+errorText);
        return false;
    }
...
//             alert("'" + (attackTag.firstChild.firstChild.nodeName  == '#text')+"'");
    ...
    if (attackTags.length){
        ...
        for each(var attackTag in attackTags){
            ...
            if (stringTag === null) {
                var errorText = "Couldn't import attack. Couldn't find attack.";
                if (sigTag !== null && sigTag.textContent.length !== 0) {
                    errorText += "The signature of the missing attack is " + sigTag.textContent;
                }
                else {
                    errorText += "The attack had no signature."
                }
                alert(errorText);
                errorCount++;
            }
                ...
                if (stringTag.childNodes.length !== 0)
                {
                    attackStringContainer.addString(
                            ...
                            sigTag!== null?sigTag.textContent:"");
        ...
        if (errorCount !== 0) {
            alert('Error processing document. '+errorCount+' error' +(errorCount>1?'s':'')+' encountered.')
        }
    }
    else {
