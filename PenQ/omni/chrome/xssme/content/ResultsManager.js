...
ResultsManager.prototype = {
    ...
    showFieldResult: function(fieldResult, showPass){
        ...
        if (showPass === false && passCount > 0) {
            ...
            rv += "This field passed " + passCount +
                    " tests. To see all the passed results, go to Tools->XSS Me->Options and click 'Show passed results in final report' and rerun this test."
            rv += "</div>";
        }
        rv += '</div>';
        rv += "</div>";
    ...
    showResults: function(testManager, errorstr){
        ...
        results += "<h2>XSS String Test Results</h2>";
        if (sortedResults.length) {
            if (errorstr !== undefined) results += "<div class='withError'>"
            for each(var fieldResult in sortedResults){
                results += this.showFieldResult(fieldResult, showPasses);
            }
            if (errorstr !== undefined) results += "<div class='withError'>"

        }
        else {
            results += "<h3>No XSS vulnerabilities found.</h3>"
        }
        results +="<div class='footer'>";
        var now = new Date();
        results += 'Results generated on ' + getMonthName(now.getMonth())  + " " + now.getDate() + ", " + now.getFullYear();
