...
if(!com.echammas.cryptofox.aes) com.echammas.cryptofox.aes={};
com.echammas.cryptofox.aes = {
...
Sbox: new Array(0x63,0x7c,0x77,0x7b,0xf2,0x6b,0x6f,0xc5,0x30,0x01,0x67,0x2b,0xfe,0xd7,0xab,0x76,
...
// Rcon is Round Constant used for the Key Expansion [1st col is 2^(r-1) in GF(2^8)] [§5.2]
	 Rcon : new Array((0x00, 0x00, 0x00, 0x00),
             				(0x01, 0x00, 0x00, 0x00),
             				(0x02, 0x00, 0x00, 0x00),
             				(0x04, 0x00, 0x00, 0x00),
             				(0x08, 0x00, 0x00, 0x00),
...
AESEncryptCtr:function(plaintext, password, nBits) {
  ...
  plaintext = this.encodeUTF8(plaintext);
  password = this.encodeUTF8(password);
  ...
  ciphertext = this.Base64encode(ciphertext);  // encode in base64
...
AESDecryptCtr:function(ciphertext, password, nBits) {
  var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
  if (!(nBits==128 || nBits==192 || nBits==256)) return '';  // standard allows 128/192/256 bit keys
  ciphertext = this.Base64decode(ciphertext);
  password = this.encodeUTF8(password);
  //var t = new Date();  // timer
  ...
  plaintext = this.decodeUTF8(plaintext);  // decode from UTF8 back to Unicode multi-byte chars
