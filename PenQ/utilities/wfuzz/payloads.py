...
class payload_permutation (payload):
	text = "permutation"
	def __init__(self,range):    ## range example --> "abcdef-4"
		payload.__init__(self)
		self.charset=[]
		try:
			ran=range.split("-")
		...
		for x in self.charset:
			set.append(x)
		words=self.xcombinations(set,self.width)
		self.listawords=[]
		for x in words:
			self.listawords.append(''.join(x))
		self.__count=len(self.listawords)

	def __iter__ (self):
		return permutation_iterator(self.listawords)
...
class permutation_iterator(base_iterator):
